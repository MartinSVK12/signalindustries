...
public class RedstoneCover extends CoverBase {
    ...
    public void tick() {
        ...
        if (sensorSlot >= 0 && machine instanceof Container && machine instanceof TileEntity tile) {
            ...
            if (tile.worldObj != null && previousState != sensorActive){
				//FIXME:
				tile.worldObj.notifyBlocksOfNeighborChange(tile.tilePos, tile.getBlock());
			}
        } else if (machine instanceof Container && machine instanceof TileEntity tile) {
    ...
    private boolean checkIfValidForSensor(ItemStack stack) {
        boolean yes = switch (sensorMode) {
	        case 0 -> stack.stackSize == sensorAmount;
	        case 1 -> stack.stackSize != sensorAmount;
	        case 2 -> stack.stackSize > sensorAmount;
	        case 3 -> stack.stackSize < sensorAmount;
	        case 4 -> stack.stackSize >= sensorAmount;
	        case 5 -> stack.stackSize <= sensorAmount;
	        default -> false;
        };
    ...
    public void onRemoved() {
        ...
        if (machine instanceof Container && machine instanceof TileEntity tile) {
            sensorActive = false;
			if (tile.worldObj != null) tile.worldObj.notifyBlocksOfNeighborChange(tile.tilePos, Blocks.AIR);
        }
    }
