...
import sunsetsatellite.signalindustries.gui.guidebook.SearchableGuidebookSubsection;
...
public class MultiblockSection extends SearchableGuidebookSubsection {
    ...
    public void reloadSection() {
        pages.clear();
        List<Field> multiblockFields = Arrays.stream(SIMultiblocks.class.getDeclaredFields()).filter((F) -> SIMultiblock.class.isAssignableFrom(F.getType())).toList();
        try {
    ...
    public List<GuidebookPage> searchPages(SearchQuery query) {
        ...
        for (GuidebookPage page : pages) {
            if (page instanceof MultiblockPage multiblockPage) {
				if (multiblockPage.multiblock.getTranslatedName().contains(query.query.getRight())) {
                    filtered.add(page);
                }
            } else if (page instanceof MultiblockMaterialsPage multiblockPage) {
				if (multiblockPage.multiblock.getTranslatedName().contains(query.query.getRight())) {
                    filtered.add(page);
                }
            }
