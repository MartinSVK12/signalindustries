...
public class MachineSection extends SearchableGuidebookSubsection {
    ...
    public MachineSection(GuidebookSection parent, List<RecipeEntryMachine> recipes, Class<? extends GuidebookPage> pageClass) {
        super(parent);
    ...
    public void reloadSection() {
        ...
        for (int i = 0; i < totalPages; i++) {
            ...
            ArrayList<RecipeEntryMachine> list = new ArrayList<>(recipes.subList(j,Math.min(j+recipesPerPage,totalRecipes)));
            try {
    ...
    public List<GuidebookPage> searchPages(SearchQuery query) {
        if(filteredPages == null || !Objects.equals(filteredPages.getLeft(), query.rawQuery)) {
            ArrayList<RecipeEntryMachine> filteredRecipes = new ArrayList<>();
            ...
            for (int i = 0; i < filteredPageCount; i++) {
                int j = i * recipesPerPage;
                ArrayList<RecipeEntryMachine> list = new ArrayList<>(filteredRecipes.subList(j, Math.min(j + recipesPerPage, filteredRecipeSize)));
                if (!list.isEmpty()) {
                    try {
