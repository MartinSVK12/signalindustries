...
import sunsetsatellite.signalindustries.gui.menus.MenuCentrifuge;
import sunsetsatellite.signalindustries.tiles.machines.simple.TileEntityCentrifuge;
import sunsetsatellite.signalindustries.util.IO;

import static sunsetsatellite.signalindustries.SignalIndustries.scene;

public class ScreenCentrifuge extends ScreenFluid {
	public Player player;
	public TileEntityCentrifuge tile;

	public ScreenCentrifuge(ContainerInventory inv, TileEntity tile) {
		super(new MenuCentrifuge(inv, (TileEntityFluidItemContainer) tile));
	...
	protected void drawGuiContainerBackgroundLayer(float f) {
		...
		Texture bg = this.mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/centrifuge_reinforced.png");
	...
	@Override
	public void init() {
		ButtonElement fluidIo = new ButtonElement(0, Math.round((float) width / 2) + 60, Math.round((float) height / 2) - 90, 20, 20, "F");
		buttons.add(fluidIo);
		ButtonElement itemIo = new ButtonElement(1, Math.round((float) width / 2) + 60, Math.round((float) height / 2) - 70, 20, 20, "I");
		buttons.add(itemIo);
