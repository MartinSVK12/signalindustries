...
import sunsetsatellite.signalindustries.gui.menus.MenuAssembler;
...
public class ScreenAssembler extends ScreenFluid {
    ...
    protected void drawGuiContainerBackgroundLayer(float f) {
        ...
        Texture bg = this.mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/basic_assembler_gui.png");
        switch (tile.tier) {
            case PROTOTYPE:
            case BASIC:
				bg = this.mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/basic_assembler_gui.png");
                break;
            case REINFORCED:
            case AWAKENED:
                break;
        ...
        if (this.tile.speedMultiplier > 1) {
            drawStringShadow(fontRenderer, this.tile.speedMultiplier + "x", x + 6, y + 6, tile.speedMultiplier >= 3 ? 0xFFFFA500 : (tile.speedMultiplier >= 2 ? 0xFFFF00FF : 0xFFFF8080));
        }
    }
    ...
    public void render(int i1, int i2, float f3) {
        ...
        FakeItemElement fakeItem = new FakeItemElement(mc);
        if (tile.recipe != null && slot.getItemStack() == null) {
            fakeItem.render(tile.recipe.getOutput(), i + slot.x, j + slot.y);
        }
    }
