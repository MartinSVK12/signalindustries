...
import sunsetsatellite.signalindustries.gui.menus.MenuCover;
...
public class ScreenRedstoneCoverConfig extends ScreenContainerAbstract {
    ...
    public void render(int mouseX, int mouseY, float partialTick) {
        ...
        if (mouseX >= pseudoSlotX - 1 && mouseX < pseudoSlotX + 16 + 1 && mouseY >= pseudoSlotY - 1 && mouseY < pseudoSlotY + 16 + 1) {
            ...
            if (grabbedItem == null && cover.sensorStack != null) {
                boolean showDescription = Keyboard.isKeyDown(29) || Keyboard.isKeyDown(157) || GameSettings.ITEM_DESCRIPTIONS.value == DescriptionPromptEnum.ALWAYS_SHOW;
    ...
    protected void drawGuiContainerBackgroundLayer(float f) {
        @NotNull Texture i = mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/sensor_pipe_gui.png");
		GLRenderer.setColor4f(1.0F, 1.0F, 1.0F, 1.0F);
    ...
    protected void drawGuiContainerForegroundLayer() {
        drawStringNoShadow(fontRenderer,"Configure: Sensor", 45, 6, 0xFF404040);
        drawStringNoShadow(fontRenderer,"Slot", 8, 8, 0x404040);
        drawStringNoShadow(fontRenderer,String.valueOf(cover.sensorAmount), 120, 42, 0x404040);
        if (cover.sensorSlot == -1) {
            drawStringNoShadow(fontRenderer,"*", 10, 42, 0x404040);
        } else {
            drawStringNoShadow(fontRenderer,String.valueOf(cover.sensorSlot), 10, 42, 0x404040);
        }
    }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseX >= pseudoSlotX - 1 && mouseX < pseudoSlotX + 16 + 1 && mouseY >= pseudoSlotY - 1 && mouseY < pseudoSlotY + 16 + 1) {
            ...
            if (EnvironmentHelper.isMultiplayerClient()) {
                NetworkHandler.sendToServer(new NetworkMessageRedstoneCoverSetFilter(new Vec3i(tile.tilePos), cover.sensorStack, tile.getClass(), cover.getDir()));
            }
        }
    }
