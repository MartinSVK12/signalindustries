...
import sunsetsatellite.signalindustries.gui.menus.MenuAutoMiner;
...
public class ScreenAutoMiner extends ScreenFluid {
    ...
    protected void drawGuiContainerBackgroundLayer(float f) {
        ...
        Texture bg = this.mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/autominer.png");
		switch (tile.tier) {
		    case PROTOTYPE, BASIC ->
			    bg = this.mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/autominer.png");
			case REINFORCED ->
			    bg = this.mc.textureManager.loadTexture("/assets/signalindustries/textures/gui/container/old/reinforced_autominer.png");
			case AWAKENED -> {
			}
	    }
    ...
    protected void drawGuiContainerForegroundLayer() {
        ...
        drawStringShadow(fontRenderer,"X: " + (tile.current.x - tile.tilePos.x), 38, 20, 0xFFFF0000);
        drawStringShadow(fontRenderer,"Y: " + (tile.current.y - tile.tilePos.y), 38, 32, 0xFF4080FF);
        drawStringShadow(fontRenderer,"Z: " + (tile.current.z - tile.tilePos.z), 38, 44, 0xFF00FF00);
        drawStringShadow(fontRenderer,"C: " + (tile.cost) + "/" + (tile.fuelMaxBurnTicks) + "t", 80, 32, 0xFF800000);
        drawStringShadow(fontRenderer,"S: " + tile.workTimer.max + "t", 80, 44, 0xFFFF8000);
        drawStringShadow(fontRenderer,"M: " + tile.multiplier, 80, 56, 0xFFFF00FF);

        if (tile.tier == Tier.REINFORCED) {
            drawStringCenteredShadow(fontRenderer,"Size", 42, 82, 0xFFFF0000);
            drawStringShadow(fontRenderer, String.valueOf(tile.size.x), 35, 100, 0xFFFFFFFF);
			drawStringShadow(fontRenderer, String.valueOf(tile.size.y), 35, 125, 0xFFFFFFFF);
        }
    }
