...
public class FakeItemElement extends Gui {
    ...
    public FakeItemElement(final Minecraft mc) {
        this.mc = mc;
    }
	...
	public void render(final ItemStack itemStack, final int x, final int y, final boolean isSelected, final Slot slot, boolean showAmount, float alpha){
		render(itemStack, x, y, isSelected, slot, showAmount, alpha, null);
	}
            ...
            if (itemStack != null) {
				...
				if(showAmount){
					if(amountOverride != null){
						if(amountOverride.length() == 5){
							GLRenderer.modelM4f().translate(4,0,0);
						}
						if(amountOverride.length() >= 6){
							GLRenderer.modelM4f().translate(7,0,0);
						}
					}
					itemModel.renderItemOverlayIntoGUI(GLRenderer.getTessellator(), this.mc.font, this.mc.textureManager, itemStack, x, y, amountOverride, 1.0f);
				}
            }
    ...
    public void render(final ItemStack itemStack, final int x, final int y, final boolean isSelected) {
        this.render(itemStack, x, y, isSelected, null, false);
    }
