...
public class PumpPage
    ...
    public List<RecipeEntryMachineFluid> recipes;
    public List<SlotGuidebook> slots;
    public Map<RecipeEntryMachineFluid,List<SlotGuidebook>> map;
    private final TooltipElement guiTooltip;
    private final ItemElement guiRenderItem;
    private static final Minecraft mc = Minecraft.getMinecraft();
    private static Player player;
    private static long ticks = 0;
    public PumpPage(GuidebookSection section, ArrayList<RecipeEntryMachineFluid> recipes) {
        super(section);
        ...
        for (RecipeEntryMachineFluid recipe : recipes) {
            ...
            if(recipe.getInput()[0].getFluidStack() != null){
                ...
                inputSymbol = new RecipeSymbol(fluidStack.toItemStack());
            } else {
            ...
            outputSymbol = new RecipeSymbol(recipe.getOutput().toItemStack());
    ...
    protected void renderForeground(TextureManager re, Font fr, int x, int y, int mouseX, int mouseY, float partialTicks) {
        drawStringCenteredNoShadow(fr, "Pump", x+width - 158 / 2, y+5, 0xFF808080);
        if(recipes.isEmpty()){
        ...
        for (int i = 1; i <= recipes.size(); i++) {
            RecipeEntryMachineFluid recipe = recipes.get(i-1);
            List<SlotGuidebook> list = map.get(recipe);
            drawStringCenteredNoShadow(fr,recipe.getData().ticks+"t",x + list.get(list.size()-1).x - 76, y +  list.get(list.size()-1).y,0xFF808080);
            drawStringCenteredNoShadow(fr,(int)(recipe.getData().cost*((float)recipe.getData().ticks/200.0f))+" sE",x + list.get(list.size()-1).x - 76, y +  list.get(list.size()-1).y + 8,0xFFCC0000);
