...
public class LaserDrillPage extends RecipePage<RecipeEntryMachineRandomOutput> {
    ...
    public Map<RecipeEntryMachineRandomOutput, List<SlotGuidebook>> map;
    ...
    public LaserDrillPage(GuidebookSection section, ArrayList<RecipeEntryMachineRandomOutput> recipes) {
        super(section);
        ...
        for (RecipeEntryMachineRandomOutput recipe : recipes) {
            ...
            int centerY = (recipeSlots.get(recipeSlots.size() - 1).y + recipeSlots.get(recipeSlots.size() - 8).y) / 2;
            recipeSlots.add(new SlotGuidebook(slotsAmount - 1, 19, centerY, recipe.getInput()[0].asNormalSymbol(), false, recipe));
