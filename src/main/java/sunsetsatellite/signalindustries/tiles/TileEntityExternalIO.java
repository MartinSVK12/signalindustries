...
public class TileEntityExternalIO extends TileEntityTieredMachineBase implements IAcceptsPosition {
    ...
    public void tick() {
        ...
        if (externalTile == null) {
            ...
            if (worldObj.getTileEntity(externalTile.tilePos.x, externalTile.tilePos.y, externalTile.tilePos.z) != externalTile) {
                externalTile = null;
    ...
    public Player getNearestPlayer() {
        ...
        for (Player p : worldObj.players) {
            int nd = (int) (Math.pow((int) p.x - tilePos.x, 2) + Math.pow((int) p.y - tilePos.y, 2) + Math.pow((int) p.z - tilePos.z, 2));
            if (dist == -1 || nd < dist) {
                ret = p;
                dist = nd;
            }
    ...
    public void receivePosition(int x, int y, int z, Side side, int dim) {
        if (tier == Tier.REINFORCED) {
            ...
            Vec3f selfPos = new Vec3f(tilePos.x, tilePos.y, tilePos.z);
            ...
            if (pos.distanceTo(selfPos) < range) {
                if (dim == worldObj.dimension.id) {
                    if (tile instanceof Container || tile instanceof IFluidInventory) {
                        if (!(tile instanceof TileEntityExternalIO)) {
                            externalTile = tile;
                            externalTileSide = Direction.getDirectionFromSide(side.id);
