            ...
            if (Arrays.stream(itemContents).anyMatch((S) -> S != null && S.stackSize > 0 && S.itemID == blockInstance.block.id())) {
                if (fluidContents[0] != null && fluidContents[0].amount >= cost) {
                    if (blockInstance.place(worldObj)) {
                        ...
                        List<ItemStack> stackList = Arrays.stream(itemContents).filter((S) -> S != null && S.stackSize > 0 && S.itemID == blockInstance.block.id()).toList();
    ...
    public void setStructureToBuild() {
        ...
        if (multiblock != null) {
            if (buildingMultiblock == null && buildingBlocks.isEmpty()) {
                ...
                ArrayList<BlockInstance> blocks = multiblock.getBlocks(new Vec3i(tilePos).add(offset), rotation);
                if (multiblock instanceof SIMultiblock){
					blocks.add(multiblock.getOrigin(new Vec3i(tilePos).add(offset), rotation));
				}
