...
public class TileEntityAssembler extends TileEntityTieredMachineBase implements IBoostable {
    ...
    public void processItem() {
        ...
        if (!(Catalyst.hasItems(recipeInputs, condensedInv))) {
            return;
        }
        int s = 0;
            ...
            for (ItemStack stack : itemContents) {
                if (symbol.matches(stack)) {
                    ...
                    if (stack.getItem().hasContainerItem()) {
                        ...
                        if (!added) {
                            EntityItem entityitem = new EntityItem(worldObj, (float) tilePos.x, (float) tilePos.y + 1, (float) tilePos.z, container);
    ...
    public boolean canProcess() {
        ...
        recipeInputs.removeIf(Objects::isNull);
        return Catalyst.hasItems(recipeInputs, condensedInv);
    }
