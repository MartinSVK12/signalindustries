...
public class TileEntityAutoMiner extends TileEntityTieredMachineBase implements IBoostable, IScreenActionListener {
    ...
    public void work() {
        ...
        if (fuelBurnTicks > 0) {
            ...
            for (int m = 0; m < multiplier; m++) {
                current.y = findTopSolidNonLiquidBlockLimited(worldObj, current.x, current.z, tilePos.y + 4);
    ...
    public void tick() {
        if (worldObj != null) {
            ...
            if (current.equals(new Vec3i())) {
                current = new Vec3i(tilePos.x - 1, tilePos.y + 4, tilePos.z + 1);
            }
            boolean silk = hasSilkTouch();
            if (silk) {
                cost = 2;
    ...
    public int findTopSolidNonLiquidBlockLimited(World world, int x, int z, int yLimit) {
        ...
        for (z &= 15; k > 0; --k) {
            ...
            Material material = l != 0 ? Blocks.blocksList[l].getMaterial() : Materials.AIR;
