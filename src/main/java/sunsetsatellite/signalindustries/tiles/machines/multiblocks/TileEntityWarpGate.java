    ...
    public void tick() {
        ...
        if (multiblock.isValid()) {
            ...
            if (state == State.CONNECTED_ONE_WAY || state == State.CONNECTED_TWO_WAY) {
                ...
                Vec3f min = new Vec3f(tilePos).subtract(offset);
                Vec3f max = new Vec3f(tilePos).subtract(offset);
                min.y -= 3;
                max.y += 3;
                min.set(axis, min.get(axis) - 3);
                max.set(axis, max.get(axis) + 3);
                Player closestPlayer = worldObj.getClosestPlayer(tilePos.x, tilePos.y, tilePos.z, 8);
