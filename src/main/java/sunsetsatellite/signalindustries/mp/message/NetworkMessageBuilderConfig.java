...
public class NetworkMessageBuilderConfig implements NetworkMessage {
	...
	public Vec3i tilePos;
    public Class<? extends TileEntity> tileClass;

    // for toggle, true = on, false = off
    public NetworkMessageBuilderConfig(Vec3i pos, Direction dir, boolean toggle, Vec3i tilePos, Class<? extends TileEntity> tileClass) {
        this.pos = pos;
        this.dir = dir;
        this.toggle = toggle;
    ...
    public void decodeFromUniversalPacket(@NotNull UniversalPacket packet) {
		...
		tilePos = new Vec3i(packet.readCompoundTag());
    ...
    public void handle(NetworkContext context) {
        if (EnvironmentHelper.isMultiplayerServer()) {
			TileEntity tileEntity = context.player.world.getTileEntity(tilePos.tilePos());
			if (tileEntity instanceof TileEntityBuilder builder && tileEntity.worldObj != null) {
